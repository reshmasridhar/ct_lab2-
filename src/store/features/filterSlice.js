...
export const filterSlice = createSlice({
    ...
    reducers: {
        ...
        handleStyle:(state, action)=>{
            if(action.payload === "All"){
                state.products = [...data];
            }else{
                const styleItems = data.filter((item)=>{
                    return item.style === action.payload;
                });
                state.products = styleItems;
            }
        }
...
export const { handleSort, handleCategory,handleStyle } =
    filterSlice.actions;